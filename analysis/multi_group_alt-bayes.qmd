...
fit <- brm(
    ...
    data = df,
    family = student(),
    control = list(adapt_delta = 0.99), # If divergence issues
    algorithm = "sampling",
    iter = 8000 
)
```

*Note: When working with bayesian models, sampling diagnostics are important to caress*
*the sampler to provide reliable posteriors. Here increasing adept_delta and the iterations*
*(length of chains) was required to get more stable estimates. Group level structures and*
*unequal variances are notoriously difficult to sample.*
...
that the groups have noticable differences in means.
...
Effect size's are reported as Log Fold Change (LogFC) and standardized effect size
(equivalent to Cohen's d) with 95% credible intervals (CI).
...
The estimated effect size is substantial, with credible intervals not spanning over a neglible effect size.

The `Elderly` group has a similar pattern, with a high probabilty of observing a higher Shannon index than the `Middle age` group.
However, the effect size is more modest, and credible intervals span over zero. Differences are likely to be neglible.

In the `Adult` vs `Elderly` comparison, we see that it is likely the `Adult` group will show a higher Shannon index.
However effect size is very modest, and the credible interval spans over zero. 

Together, these results would suggest that the `Middle-aged` group differs significantly from the `Adult` group,
with a lower Shannon index. While showing a similar pattern when compared to the `Elderly` group, however
...
in the Shannon index amoungs the groups. After observing a significant result, it is typical to further
...
# 6. Effect size comparsions


# 7. Benchmarking speed
...
mb_res <- microbenchmark(
    ...
    .anova(df),
    .kruskal_test(df),
    times = 3L, # adjust for more repetitions
    unit = "seconds"
)
```
