...
diff_am <- re[, "Adult"] - re[, "Middle_age"]
diff_ae <- re[, "Adult"] - re[, "Elderly"]
diff_em <- re[, "Elderly"] - re[, "Middle_age"]
# Probability of direction
pd_am <- p_direction(diff_am)
pd_ae <- p_direction(diff_ae)
...
fc_am <- log2(post_a / post_m)
fc_ae <- log2(post_a / post_e)
fc_em <- log2(post_e / post_m)
ci_am <- quantile(fc_am, probs = c(0.05, 0.95))
ci_ae <- quantile(fc_ae, probs = c(0.05, 0.95))
ci_em <- quantile(fc_em, probs = c(0.05, 0.95))
# Collect into a dataframe
tbl <- tibble(
  comp = c("Adult - Middle_age", "Adult - Elderly", "Elderly - Middle_age"),
  pd = c(pd_am$pd, pd_ae$pd, pd_em$pd),
  p_like = c(pd_to_p(pd_am$pd), pd_to_p(pd_ae$pd), pd_to_p(pd_em$pd)),
  log2fc = c(mean(fc_am), mean(fc_ae), mean(fc_em)),
  ci_low = c(ci_am[1], ci_ae[1], ci_em[1]),
  ci_upper = c(ci_am[2], ci_ae[2], ci_em[2])
)


table2 <- tbl %>%
  mutate(ci = sprintf("[%.2f, %.2f]", ci_low, ci_upper)) %>%
  select(comp, log2fc, ci, pd, p_like) %>%
  gt() %>%
  tab_header(
    title = "Pairwise Comparisons of Groups",
    subtitle = "Posterior probability of direction and effect size"
  ) %>%
  fmt_number(
    columns = c(log2fc, pd),
    decimals = 2
...
library(gt)
# ensure it's a gt_tbl
inherits(table2, "gt_tbl")

# Step 1: HTML (self-contained)
gt::gtsave(table2, "table.html", inline_css = TRUE)

# Step 2: PNG via webshot2 (try selector first, then fallback)
tryCatch(
  webshot2::webshot("table.html", file = "table.png", selector = ".gt_table", zoom = 2),
  error = function(e) webshot2::webshot("table.html", file = "table.png", zoom = 2)
)
